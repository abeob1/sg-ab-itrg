...
Namespace AE_ITRG_AO01
    ...
    Module modCommon
        ...
        Function HeaderValidation(FormUID As SAPbouiCOM.Form, ByRef sErrDesc As String) As Long
            ...
            Dim oPostPeriod As SAPbouiCOM.CheckBox
            Dim oExchPrice As SAPbouiCOM.CheckBox
            Dim oPricelist As SAPbouiCOM.CheckBox
            Dim oBinLocation As SAPbouiCOM.CheckBox
            Dim oFlag As Boolean = False
            Dim oFlag1 As Boolean = False

            oDT_Entities = New DataTable

            oDT_Entities.Columns.Add("SNo", GetType(String))
            oDT_Entities.Columns.Add("UniqueNo", GetType(String))
            oDT_Entities.Columns.Add("TransType", GetType(String))
            oDT_Entities.Columns.Add("Code", GetType(String))
            oDT_Entities.Columns.Add("Name", GetType(String))
            oDT_Entities.Columns.Add("SyncStatus", GetType(String))
            oDT_Entities.Columns.Add("SyncDate", GetType(String))
            oDT_Entities.Columns.Add("SyncErrMsg", GetType(String))
            Try

                oPricelist = FormUID.Items.Item("Chk_Prices").Specific
                oPostPeriod = FormUID.Items.Item("chk_post").Specific
                oExchPrice = FormUID.Items.Item("Chk_exch").Specific
                oBinLocation = FormUID.Items.Item("Chk_bin").Specific
                If oPricelist.Checked = True Then oFlag2 = True
                If oPostPeriod.Checked = True Then oFlag3 = True
                If oExchPrice.Checked = True Then oFlag4 = True
                If oBinLocation.Checked = True Then oFlag5 = True

                sFuncName = "Validation()"
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Starting function", sFuncName)

                If FormUID.Items.Item("c_masttype").Specific.value.ToString.Trim() = "" Then
                    ...
                    FormUID.ActiveItem = "c_masttype"
                    Return RTN_ERROR
                End If

                If oMatrix.RowCount = 0 And oFlag2 = False And oFlag3 = False And oFlag4 = False And oFlag5 = False Then
                    p_oSBOApplication.StatusBar.SetText("No Data to Replicate.... ....... !", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                    Return RTN_ERROR
                End If

                For i As Integer = 1 To oMatrix.RowCount
                    oCheckbox = oMatrix.Columns.Item("Select").Cells.Item(i).Specific
                    If oCheckbox.Checked = True Then
                        oFlag = True
                        oDT_Entities.Rows.Add(i, oMatrix.Columns.Item("UniqueId").Cells.Item(i).Specific.String, oMatrix.Columns.Item("TransType").Cells.Item(i).Specific.String, oMatrix.Columns.Item("Code").Cells.Item(i).Specific.String, oMatrix.Columns.Item("Name").Cells.Item(i).Specific.String, "", "", "")
                        '-'''oDT_Entities.Rows(i - 1)("SyncStatus") = "NO"
                        If oMatrix.Columns.Item("SyncStatus").Cells.Item(i).Specific.String = "YES" Then
                            oFlag1 = True
                        End If
                    End If
                Next i

                If oFlag = False And oFlag2 = False And oFlag3 = False And oFlag4 = False And oFlag5 = False Then
                    p_oSBOApplication.StatusBar.SetText("No Transaction being selected to Replicate.... ....... !", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
                    Return RTN_ERROR
                End If

                If oFlag1 = True Then
                    p_oSBOApplication.StatusBar.SetText("Refresh the transactions..... ....... !", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
                    Return RTN_ERROR
                End If
...
#End Region
        ...
        Public Function GetDocumentLine(ByRef oBaseCompany As SAPbobsCOM.Company, DocEntry As Integer, TableName As String) As SAPbobsCOM.Recordset
            Dim ors As SAPbobsCOM.Recordset = Nothing
            Try
                ors = oBaseCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                ors.DoQuery(String.Format("select ""LineNum"", ""ItemCode"", ""Dscription"", ""Text"", ""Quantity"", ""PriceBefDi"", ""DiscPrcnt"", ""WhsCode"",""Currency"",""BaseLine"" from {0} where ""DocEntry"" = {1}", TableName, DocEntry))
                ors.MoveFirst()
                Return ors
            Finally
                ors = Nothing
            End Try
        End Function
        Public Function GetDocumentLine1(ByRef oBaseCompany As SAPbobsCOM.Company, DocEntry As Integer, TableName As String) As SAPbobsCOM.Recordset
            Dim ors As SAPbobsCOM.Recordset = Nothing
            Try
                ors = oBaseCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                ors.DoQuery(String.Format("select ""LineSeq"", ""AftLineNum"", ""OrderNum"", ""LineType"", ""LineText"" from {0} where ""DocEntry"" = {1}", TableName, DocEntry))
                ors.MoveFirst()
                Return ors
            Finally
                ors = Nothing
            End Try
        End Function
        Public Function GetDocumentLine3(ByRef oBaseCompany As SAPbobsCOM.Company, DocEntry As Integer, TableName As String, DetailTable As String) As SAPbobsCOM.Recordset
            Dim ors As SAPbobsCOM.Recordset = Nothing
            Try
                ors = oBaseCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                ors.DoQuery(String.Format("select a.""LineNum"", a.""ItemCode"", a.""Dscription"", a.""Text"", a.""Quantity"", a.""PriceBefDi"", a.""DiscPrcnt"", a.""WhsCode"",a.""Currency"",a.""BaseLine"", b.""VisOrder"" from {0} A inner join {1} B on  a.""BaseEntry"" = b.""DocEntry"" and a.""BaseLine""= b.""LineNum"" and a.""BaseType"" = b.""ObjType"" where  a.""DocEntry"" = {2}", TableName, DetailTable, DocEntry))
                ors.MoveFirst()
                Return ors
            Finally
                ors = Nothing
            End Try
        End Function
        Public Function ConnectToTargetCompany(ByRef oTargetCompany As SAPbobsCOM.Company, _
            ...
            Try
                ...
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Assigning the Representing Database Details", sFuncName)
                Dim ser As String = p_oDICompany.Server
                Dim Licser As String = p_oDICompany.LicenseServer
                oTargetCompany.Server = p_oDICompany.Server
                oTargetCompany.DbServerType = SAPbobsCOM.BoDataServerTypes.dst_HANADB
                oTargetCompany.LicenseServer = p_oDICompany.LicenseServer
                oTargetCompany.CompanyDB = sEntity
                oTargetCompany.DbUserName = "SYSTEM"
                oTargetCompany.DbPassword = "Sapb1hana"
                oTargetCompany.UserName = sUsername
                oTargetCompany.Password = sPassword
                oTargetCompany.language = SAPbobsCOM.BoSuppLangs.ln_English
                oTargetCompany.UseTrusted = False
                ...
                iRetValue = oTargetCompany.Connect()
                If iRetValue <> 0 Then
                    oTargetCompany.GetLastError(iErrCode, sErrDesc)

                    sErrDesc = String.Format("Connection to Database ({0}) {1} {2} {3}", _
                        oTargetCompany.CompanyDB, System.Environment.NewLine, _
                                    vbTab, sErrDesc)
                ...
                Console.WriteLine("Company Connected Successfully..", sFuncName)
                ConnectToTargetCompany = RTN_SUCCESS
            Catch ex As Exception
                sErrDesc = ex.Message
                Call WriteToLogFile(ex.Message, sFuncName)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Company Not Connected.." & ex.Message, sFuncName)
                Console.WriteLine("Completed with ERROR ", sFuncName)
