...
--select :object_type , :list_of_key_cols_tab_del from dummy;
--------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
--- Item Groups - create/update to integration table.......... by Senthil kumar. G
---------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
--- BOM - create/update to integration table.......... by Senthil kumar. G
---------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
--- Item Master data Vendor Default - create/update to integration table.......... by Senthil kumar. G
---------------------------------------------------------------------------------------------------------------------------------
if :object_type='4' and (:transaction_type='A' or :transaction_type='U') then

SELECT count(T0."CODE") into cnt FROM "INTEGRATION" T0 WHERE T0."CODE" = :list_of_cols_val_tab_del AND IFNULL(T0."SYNCSTATUS",'NO') = 'NO' and T0."MASTERTYPE" = 'ITEMMASTER' and T0."TRANSTYPE"='VendorDef';
	if :cnt=0 then
		select count("UNIQUEID")+1 into cnt from "INTEGRATION";
		select "ItemName" into name from "OITM" T1 WHERE T1."ItemCode" =  :list_of_cols_val_tab_del;
			if :cnt>0 then
				insert into "INTEGRATION" ("UNIQUEID","MASTERTYPE","TRANSTYPE","CODE", "NAME", "SYNCSTATUS")values 
				(cnt,'ITEMMASTER','VendorDef',:list_of_cols_val_tab_del,name,'NO') ;  
			end if;
	end if;
end if;
